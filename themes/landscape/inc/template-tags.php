 ...
 * @since landscape 1.0
 */
function landscape_content_nav( $nav_id ) {
	...
	<nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
		<h1 class="assistive-text"><?php _e( 'Post navigation', 'landscape' ); ?></h1>

	<?php if ( is_single() ) : // navigation links for single posts ?>

		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'landscape' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'landscape' ) . '</span>' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'landscape' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'landscape' ) ); ?></div>
		<?php endif; ?>
...
endif; // landscape_content_nav

if ( ! function_exists( 'landscape_comment' ) ) :
/**
 ...
 * @since landscape 1.0
 */
function landscape_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'landscape' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '(Edit)', 'landscape' ), ' ' ); ?></p>
	<?php
			break;
		default :
	?>
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<footer>
				<div class="comment-author vcard">
					...
					<?php printf( __( '%s <span class="says">says:</span>', 'landscape' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
				</div><!-- .comment-author .vcard -->
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em><?php _e( 'Your comment is awaiting moderation.', 'landscape' ); ?></em>
					<br />
				<?php endif; ?>
				...
				<div class="comment-meta commentmetadata">
					...
					<?php
						...
						printf( __( '%1$s at %2$s', 'landscape' ), get_comment_date(), get_comment_time() ); ?>
					</time></a>
					<?php edit_comment_link( __( '(Edit)', 'landscape' ), ' ' );
					?>
...
endif; // ends check for landscape_comment()
